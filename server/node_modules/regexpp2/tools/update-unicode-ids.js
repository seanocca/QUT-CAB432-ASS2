...
(async () => {
    ...
    await processEachLine(line => {
        let m; //eslint-disable-line init-declarations
        if (banner === "") {
...
${makeCondition(idContinueLarge, Mode.Former)}
    ...
    const engine = new eslint_1.CLIEngine({ fix: true });
...
function save(content) {
    return new Promise((resolve, reject) => {
        fs_1.default.writeFile(FILE_PATH, content, error => (error ? reject(error) : resolve()));
    });
}
